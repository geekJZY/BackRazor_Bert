...
# from src.transformers.models.bert.modeling_bert import BertSelfAttention, BertIntermediate
...
def count_activation_size(net, input, require_backward=True, activation_bits=32, head_only=False):
	...
	def add_hooks(m_):
		...
		if type(m_) in [nn.Conv1d, nn.Conv2d, nn.Conv3d, ]:
			fn = count_convNd
		elif type(m_) in [nn.Linear, nn.Embedding, LinearSparse]:
			fn = count_linear
		...
		if isinstance(m, BertSelfAttention):
			def new_forward(_module):
				def lambda_forward(*args, **kwargs):
					...
					isbackRazor = _module.backRazor
					if isbackRazor and _module.query.quantize:
						backward_act_byte = 1
					else:
					...
					if isbackRazor:
						attn_prune_ratio = _module.query.masker.prune_ratio
					...
					result = _module.old_forward(*args, **kwargs)
		...
		if isinstance(m, BertIntermediate):
			def new_forward(_module):
				def lambda_forward(_x):
					...
					memory_info_dict['grad_activation_size'] += _module.dense.out_features * n_tokens * backward_act_byte
...
def profile_memory_cost(net, input, require_backward=True, head_only=False,
	...
	activation_size, grad_activation_size = count_activation_size(net, input, require_backward, activation_bits, head_only)

	# TODO: change count activation
