...
def main():
    ...
    config.self_slimming = False
    config.inter_slimming = False
    ...
    if training_args.do_train:
        ...
        if training_args.prune_before_train:
            ...
            for m in bert_layers:
                # random.shuffle(a)
                l = torch.chunk(torch.sum(m.intermediate.dense.weight.data.abs(), 1), 12)
                l = torch.stack([torch.sum(ll) for ll in l])
                pruned_inter_neurons = torch.argsort(l)[:4]

                # pruned_inter_neurons = a[:3]# [i for i in range(new_config.intermediate_size) if mask[i] == 0]
            ...
            for m in attention_modules:
                ...
                # a = list(range(12))
                # random.shuffle(a)
                l = torch.chunk(torch.sum(m.self.query.weight.data.abs(), 1) + torch.sum(m.self.value.weight.data.abs(), 1) + torch.sum(m.self.key.weight.data.abs(), 1), 12)
                l = torch.stack([torch.sum(ll) for ll in l])
                pruned_heads = torch.argsort(l)[:4]
                # pruned_heads = a[:3]
