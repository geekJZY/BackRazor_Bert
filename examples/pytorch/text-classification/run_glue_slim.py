...
def main():
    ...
    if training_args.do_train:
        ...
        train_result = trainer.train(resume_from_checkpoint=checkpoint,
            config=config,
            save_self_slimming=True, save_inter_slimming=True,
            l1_loss_self_coef=training_args.l1_loss_self_coef,
            l1_loss_inter_coef=training_args.l1_loss_inter_coef)
        ...
        self_slimming_coef_records = train_result.self_slimming_coef_records
        assert len(self_slimming_coef_records) == config.num_hidden_layers
        for i, slimming_coef in enumerate(self_slimming_coef_records):
            self_slimming_coef_records[i] = np.stack(slimming_coef, axis=0)
        np.save(os.path.join(training_args.output_dir, 'self_slimming_coef_records.npy'),
                np.stack(self_slimming_coef_records, axis=0))

        # save intermediate layer slimming coefficients
        inter_slimming_coef_records = train_result.inter_slimming_coef_records
        assert len(inter_slimming_coef_records) == config.num_hidden_layers
        for i, slimming_coef in enumerate(inter_slimming_coef_records):
            inter_slimming_coef_records[i] = np.stack(slimming_coef, axis=0)
        np.save(os.path.join(training_args.output_dir, 'inter_slimming_coef_records.npy'),
